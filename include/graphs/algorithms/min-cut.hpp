...
public:
    MinCut(const F &maxFlow, const G &graph) : m_minCut(maxFlow.maxFlow())
    {
        ...
        for (auto edge : graph.edges()) {
            ...
            if (flow > 0) { g.addEdge(edge.vertex2, edge.vertex1); }
            if (flow < capacity) { g.addEdge(edge.vertex1, edge.vertex2); }
        }

        BFS<Graph> bfs(g, maxFlow.source());

        for (auto edge : graph.edges()) {
            if (bfs.distTo(edge.vertex1) != INT_MAX
                && bfs.distTo(edge.vertex2) == INT_MAX) {
                m_cut.push_back(edge);
            }
        }
    }

    typename G::Weight minCut() const { return m_minCut; }

    const std::vector<typename G::Edge> &cutEdges() const { return m_cut; }
